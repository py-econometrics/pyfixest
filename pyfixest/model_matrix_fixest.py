...
def model_matrix_fixest(
    ...
    tuple
        ...
        - X (pd.DataFrame): A DataFrame of the covariates. If `combine = True`, contains covariates and fixed effects as dummies.
        - I (Optional[pd.DataFrame]): A DataFrame of the Instruments, None if no IV.
        - fe (Optional[pd.DataFrame]): A DataFrame of the fixed effects, None if no fixed effects specified. Only applicable if `combine = False`.
        - na_index (np.array): An array with indices of dropped columns.
        - fe_na (np.array): An array with indices of dropped columns due to fixed effect singletons or NaNs in the fixed effects.
        - na_index_str (str): na_index, but as a comma-separated string. Used for caching of demeaned variables.
        - z_names (Optional[List[str]]): Names of all covariates, minus the endogenous variables, plus the instruments. None if no IV.
        - weights (Optional[str]): Weights as a string if provided, or None if no weights, e.g., "weights".
        - has_weights (bool): A boolean indicating whether weights are used.
        - icovars (Optional[List[str]]): A list of interaction variables provided via `i()`. None if no interaction variables via `i()` provided.

    Attributes
    ----------
    icovars : list or None
...
def _check_is_iv(fml):
    ...
    """
    Check if the formula contains an IV (Instrumental Variable) by checking if
    the formula contains one or two '~'.
...
def _clean_fe(data: pd.DataFrame, fval: str) -> Tuple[pd.DataFrame, List[int]]:
    ...
    This is a helper function used in `model_matrix_fixest()`. The function converts
    the fixed effects to integers and marks fixed effects with NaNs. It's important
    to note that NaNs are not removed at this stage; this is done in `model_matrix_fixest()`.
...
def _get_icovars(_ivars: List[str], X: pd.DataFrame) -> Optional[List[str]]:
    ...
    Note that `icovars` is a shorthand for "interaction covariates".
...
def _check_i_refs2(ivars, i_ref1, i_ref2, data) -> None:
    ...
    """
    Check if the reference levels have the same type as the variable (if not, string matching might fail).
    If not, string matching for dropping reference levels might fail.
