...
class SensitivityAnalysis:
    ...
    def robustness_value(
        self, X: Optional[str] = None, q: float = 1, alpha: float = 1.0
    ...
    def sensitivity_stats(
        self, X: Optional[str] = None, q: float = 1, alpha: float = 0.05
    ) -> dict:
        """
    ...
    def ovb_bounds(
        ...
        bounds = self._ovb_bounds_partial_r2(
            treatment=treatment, benchmark_covariates=benchmark_covariates, kd=kd, ky=ky
        )

        if adjusted_estimate:
            bounds["treatment"] = treatment
            bounds["adjusted_estimate"] = self.adjusted_estimate(
                bounds["r2dz_x"], bounds["r2yz_dx"], treatment=treatment, reduce=True
            )
            bounds["adjusted_se"] = self.adjusted_se(
                bounds["r2dz_x"], bounds["r2yz_dx"], treatment=treatment
            )
            bounds["adjusted_t"] = self.adjusted_t(
                bounds["r2dz_x"],
                bounds["r2yz_dx"],
                treatment=treatment,
                reduce=True,
            ...
            bounds["adjusted_lower_CI"] = (
                bounds["adjusted_estimate"] - se_multiple * bounds["adjusted_se"]
            )
    ...
    def _ovb_bounds_partial_r2(
        ...
        if model is None or treatment is None:
            raise ValueError(
                "ovb_partial_r2 requires a model object and a treatment variable"
            )
        if kd is None:
            kd = [1, 2, 3]
        ...
        if treatment not in X.columns:
            raise ValueError(
                f"Treatment '{treatment}' not found in model coefficients."
            )

        non_treatment = X.drop(columns=treatment)
        covariate_names = [col for col in non_treatment.columns if col != "Intercept"]
        covariates = " + ".join(covariate_names)

        formula = f"{treatment} ~ {covariates}"

        treatment_model = feols(formula, data=data)
        if isinstance(treatment_model, FixestMulti):
            raise TypeError(
                "Internal error: expected a single model for treatment regression."
            )
        ...
        kd_list = [float(x) for x in kd] if isinstance(kd, list) else [float(kd)]

        if ky is None:
            ky_list = kd_list.copy()
        elif isinstance(ky, list):
            ky_list = [float(x) for x in ky]
        else:
            ky_list = [float(ky)]

        if len(ky_list) != len(kd_list):
            if len(ky_list) == 1:
                ky_list = ky_list * len(kd_list)
            else:
                raise ValueError("ky must have length 1 or the same length as kd.")
        ...
        for b in benchmark_covariates:
            ...
            for kd_val, ky_val in zip(kd_list, ky_list):
                r2dz_x = kd_val * (r2dxj_x / (1 - r2dxj_x))

                if r2dz_x >= 1:
                    raise ValueError(
                        f"Implied bound on r2dz.x >= 1 for benchmark {b} with kd={kd_val}."
                        "Impossible scenario. Try a lower kd."
                    )
                r2zxj_xd = (
                    kd_val * (r2dxj_x**2) / ((1 - kd_val * r2dxj_x) * (1 - r2dxj_x))
                )

                if r2zxj_xd >= 1:
                    raise ValueError(
                        f"Impossible kd value for benchmark {b}. Try a lower kd."
                    )

                r2yz_dx = (
                    ((np.sqrt(ky_val) + np.sqrt(r2zxj_xd)) / np.sqrt(1 - r2zxj_xd)) ** 2
                ) * (r2yxj_dx / (1 - r2yxj_dx))

                if r2yz_dx > 1:
                    print(
                        f"Warning: Implied bound on r2yz.dx > 1 for {b}. Capping at 1."
                    )
                    r2yz_dx = 1.0
    ...
    def adjusted_estimate(
        ...
        if reduce:
            return np.sign(estimate) * (
                abs(estimate) - self.bias(r2dz_x, r2yz_dx, treatment=treatment)
            )
        else:
            return np.sign(estimate) * (
                abs(estimate) + self.bias(r2dz_x, r2yz_dx, treatment=treatment)
    ...
    def adjusted_t(
        ...
        """
        new_estimate = self.adjusted_estimate(
            r2dz_x, r2yz_dx, treatment=treatment, reduce=reduce
        )
    ...
    def summary(
        ...
        h0 = (1 - q) * est
        print(
            f"-- The null hypothesis deemed problematic is H0:tau = {h0:.{decimals}f} \n"
        )
        ...
        if benchmark_covariates is not None:
            ...
            print(
                bounds_df[cols].to_string(
                    index=True,
                    float_format=lambda x: f"{x:.{6}f}"
                    if abs(x) < 1e-3
                    else f"{x:.{decimals}f}",
                )
            )
        ...
        **kwargs
        ...
        if plot_type == "contour":
            return ovb_contour_plot(
                sens=self, treatment=treatment, sensitivity_of=sensitivity_of, **kwargs
            )
        elif plot_type == "extreme" and sensitivity_of == "t-value":
            raise NotImplementedError(
                "Extreme sensitivity plots for t-values not yet implemented."
            )
        elif plot_type == "extreme":
            return ovb_extreme_plot(sens=self, treatment=treatment, **kwargs)
        else:
