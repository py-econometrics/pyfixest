...
class OLSJAX:
    ...
    def __init__(self, X: jax.Array, Y: jax.Array, fe: Optional[jax.Array] = None, vcov: str = 'iid'):
    ...
    def fit(self):
        ...
        self.Y, self.X = self.demean(Y = self.Y_orignal, X = self.X_orignal, fe = self.fe, weights = self.weights)
    ...
    def demean(self, Y: jax.Array, X: jax.Array, fe: jax.Array, weights: jax.Array):
        ...
        if fe is not None:
            ...
            YXd, success = demean_jax(x = YX, flist = fe, weights = self.weights, output = "jax")
