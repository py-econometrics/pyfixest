...
class GelbachDecomposition:
    ...
    def coefplot(
        ...
        try:
            ...
            raise ImportError("matplotlib is required for coefplot. Install with: pip install matplotlib")
        ...
        if "direct_effect" not in levels.index or "full_effect" not in levels.index:
            raise ValueError("Expected 'direct_effect' and 'full_effect' in 'Levels (units)'.")
        ...
        exclude = {"direct_effect", "full_effect", "unexplained_effect", "explained_effect"}
        ...
        if keep is not None or drop is not None:
            ...
            mediators = _select_order_coefs(mediators, keep_list, drop_list, exact_match)
        ...
        # Get mediator values
        mediator_values = [
            float(levels.loc[med, "coefficients"])
            for med in ordered_mediators
        ]

        # Calculate explained effect
        explained_effect = direct_effect - full_effect

        # Create waterfall positions
        # [Initial, Med1, Med2, ..., MedN, Unexplained]
        n_components = len(ordered_mediators)
        positions = list(range(n_components + 2))  # 0, 1, 2, ..., N, N+1

        # Values for each bar: [direct, med1, med2, ..., medN, unexplained]
        bar_values = [direct_effect] + mediator_values + [full_effect]

        # Calculate cumulative positions for proper waterfall
        bar_bottoms = []
        bar_heights = []
        bar_types = []  # Track bar types for styling

        # Initial bar starts from 0
        bar_bottoms.append(0)
        bar_heights.append(direct_effect)
        bar_types.append("initial")

        # Mediator bars: each starts where previous ended, moving toward full_effect
        cumulative_position = direct_effect
        for val in mediator_values:
            if val >= 0:
                # Positive contribution: bar goes down from current position
                bar_bottoms.append(cumulative_position - val)
                bar_heights.append(val)
                bar_types.append("mediator_positive")
            else:
                # Negative contribution: bar goes up from current position
                bar_bottoms.append(cumulative_position)
                bar_heights.append(abs(val))
                bar_types.append("mediator_negative")
            cumulative_position -= val

        # Final bar (unexplained effect) starts from 0
        bar_bottoms.append(0)
        bar_heights.append(full_effect)
        bar_types.append("unexplained")

        # Colors based on bar types
        colors = []
        for bar_type in bar_types:
            if bar_type == "initial":
                colors.append("#1f77b4")  # Blue for initial
            elif bar_type == "mediator_positive":
                colors.append("#7bcf7b")  # Lighter green for positive mediators (better contrast)
            elif bar_type == "mediator_negative":
                colors.append("#d62728")  # Red for negative mediators
            elif bar_type == "unexplained":
                colors.append("#1f77b4")  # Blue for unexplained
        ...
        # Draw bars
        bars = ax.bar(positions, bar_heights, bottom=bar_bottoms, width=0.6, color=colors, alpha=0.8)

        # Set y-axis limits to make room for spanner
        y_max = max(direct_effect, full_effect) * 1.15
        ax.set_ylim(0, y_max)
        ...
        if n_components > 0:
            ...
            spanner_y = direct_effect + (y_max - direct_effect) * 0.3  # Position above bars

            # Draw horizontal line spanning mediator bars
            ax.plot([mediator_start - 0.3, mediator_end + 0.3], [spanner_y, spanner_y],
                   color='#2E4A87', linewidth=2)  # Navy blue for better appearance

            # Draw vertical lines at ends
            tick_height = y_max * 0.015
            ax.plot([mediator_start - 0.3, mediator_start - 0.3],
                   [spanner_y - tick_height, spanner_y + tick_height],
                   color='#2E4A87', linewidth=2)
            ax.plot([mediator_end + 0.3, mediator_end + 0.3],
                   [spanner_y - tick_height, spanner_y + tick_height],
                   color='#2E4A87', linewidth=2)
            ...
            if annotate_shares:
                ...
                spanner_label = f"Explained Effect: {explained_effect:.3f} ({share_of_direct:.1f}%)"
            else:
                spanner_label = f"Explained Effect: {explained_effect:.3f}"

            ax.text(spanner_center, spanner_y + tick_height * 2.5, spanner_label,
                   ha="center", va="bottom", color='#2E4A87', fontweight="bold", fontsize=10)

        # Add value labels on bars
        for i, (pos, height, bottom, val, bar_type) in enumerate(zip(positions, bar_heights, bar_bottoms, bar_values, bar_types)):
            ...
            if bar_type == "initial":
                label = f"{val:.3f}"
                ax.text(pos, label_y, label, ha="center", va="center",
                       color="black", fontweight="bold", fontsize=10)

            elif bar_type in ["mediator_positive", "mediator_negative"]:
                if annotate_shares:
                    ...
                    # Smart spacing: use fixed spacing based on reference, but don't exceed bar height
                    # Calculate reference spacing from the largest bar or a reasonable default
                    reference_spacing = 0.12  # Base spacing in data units
                    max_spacing_for_bar = height * 0.35  # Don't use more than 35% of bar height per gap
                    actual_spacing = min(reference_spacing, max_spacing_for_bar)

                    if len(lines) == 1:
                        # Just center the single value
                        ax.text(pos, label_y, lines[0], ha="center", va="center",
                               color="black", fontweight="bold", fontsize=10)
                    elif len(lines) == 2:
                        # Top and bottom with smart spacing
                        ax.text(pos, label_y + actual_spacing/2, lines[0], ha="center", va="center",
                               color="black", fontweight="bold", fontsize=10)
                        ax.text(pos, label_y - actual_spacing/2, lines[1], ha="center", va="center",
                               color="black", fontweight="bold", fontsize=10)
                    elif len(lines) == 3:
                        # Top, middle, bottom with smart spacing
                        ax.text(pos, label_y + actual_spacing, lines[0], ha="center", va="center",
                               color="black", fontweight="bold", fontsize=10)
                        ax.text(pos, label_y, lines[1], ha="center", va="center",
                               color="black", fontweight="bold", fontsize=10)
                        ax.text(pos, label_y - actual_spacing, lines[2], ha="center", va="center",
                               color="#2E4A87", fontweight="bold", fontsize=10)
                else:
                    # Just the main value
                    ax.text(pos, label_y, f"{val:.3f}", ha="center", va="center",
                           color="black", fontweight="bold", fontsize=10)
        ...
        # Customize the plot
        plot_labels = (["Initial Difference"] +
                      [display_labels[med].replace("_", " ") for med in ordered_mediators] +
        ...
        if notes is not None:
            plt.figtext(0.02, 0.02, notes, fontsize=9, style='italic',
            ...
            plt.figtext(0.02, 0.02, default_note, fontsize=9, style='italic',
                       wrap=True, ha='left', va='bottom')
            plt.tight_layout()
            plt.subplots_adjust(bottom=0.20)  # More space to avoid overlap with x-labels
        else:
            # No notes by default
            plt.tight_layout()
        plt.show()
