...
from pyfixest.estimation.fepois_ import Fepois, _fepois_input_checks
...
class Feglm(Fepois, ABC):
    ...
    def get_fit(self):
        ...
        for r in range(_maxiter):
            ...
            v_tilde = self.get_v_tilde(y = _Y.flatten(), mu = mu, W_tilde = W_tilde.flatten(), detadmu = detadmu)

            # Step 3 compute v_dotdot(r-1) and X_dotdot(r-1) - demeaning
            v_dotdot, X_dotdot = self.residualize(v = v_tilde, X = X_tilde, flist = _fe, weights = W_tilde.flatten(), tol = _fixef_tol)
    ...
    def get_beta_diff(self, X_dotdot, v_dotdot):
        ...
        #beta_diff = np.linalg.lstsq(X_dotdot, v_dotdot.reshape(-1, 1), rcond = None)[0].flatten()
        beta_diff = np.linalg.solve(X_dotdot.T @ X_dotdot, X_dotdot.T @ v_dotdot)
        return beta_diff

    def get_eta_update(self, W_tilde, v_tilde, v_dotdot, X_dotdot, beta_diff, eta):
        "Get the eta update (formula 4.5)."
        #import pdb; pdb.set_trace()
        return  (v_tilde  - X_dotdot @ beta_diff) / W_tilde + eta
    ...
    def residualize(self, v, X, flist, weights, tol):
        ...
        if flist is None:
            ...
            vX_resid, success = demean(x = np.c_[v, X], flist = flist, weights = weights, tol = tol)
            if success is False:
    ...
    def update_eta_step_halfing(self, Y, beta, eta, mu, deviance, beta_update_diff, W_tilde, v_tilde, v_dotdot, X_dotdot, deviance_old, step_halfing_tolerance):
