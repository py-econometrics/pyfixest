<!-- Generated from docs/contributing.qmd; do not edit. -->
...
Start with the repository-maintained [PyFixest architecture guide](architecture.md)
for dependency boundaries, the estimation pipeline, model capabilities, cache and
state lifecycles, the Python/Rust boundary, and the numerical test map. DeepWiki
can be a useful secondary aid, but the local guide and source code are the sources
of truth.
