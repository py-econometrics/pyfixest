<!-- Generated from docs/quickstart.qmd; do not edit. -->
...
Alternatively, the `.summarize` module contains a `summary` function, which can be applied on instances of regression model objects or lists of regression model objects. For details on how to customize `etable()`, please take a look at the [dedicated vignette](tutorials/regression-tables.md).
...
- `CRV1` and `CRV3`: cluster robust standard errors according to Cameron, Gelbach, and Miller (2011). See A Practitioner's Guide to Cluster-Robust Inference *(binary asset omitted)*.  For   `CRV1` and `CRV3` one should pass a dictionaty of the form `{"CRV1": "clustervar"}`.
...
It is also possible to run a wild (cluster) bootstrap after estimation (via the [wildboottest module](https://github.com/py-econometrics/wildboottest), see MacKinnon, J. G., Nielsen, M. Ø., & Webb, M. D. (2023). Fast and reliable jackknife and bootstrap methods for cluster-robust inference. Journal of Applied Econometrics, 38(5), 671–694. *(binary asset omitted)*):
...
You can also conduct inference via randomization inference (see Heß, Stata Journal 2017) *(binary asset omitted)*.
...
You can access the first-stage regression through the public `first_stage_model` property:

```python
pf.etable([iv_fit.first_stage_model, iv_fit])
```

After `first_stage()`, the public `first_stage_f_statistic` property gives the first-stage F statistic:

```python
iv_fit.first_stage_f_statistic
```
...
For details, take a look at the dedicated [quantreg vignette](tutorials/quantile-regression.md).
...
`feols()` and `fepois` returns an instance of a `FixestMulti` object, which essentially consists of a dicionary of `Fepois` or [Feols](reference/estimation.models.feols_.Feols.md) instances.
...
For more details see the vignette on [Difference-in-Differences Estimation](tutorials/difference-in-differences.md).
