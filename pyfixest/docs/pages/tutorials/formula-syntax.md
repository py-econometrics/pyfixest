...
For details on fixed effects regression, take a look at the [OLS with Fixed Effects](ols-fixed-effects.md) vignette.
...
For details on IV estimation, take a look at the [Instrumental Variables](instrumental-variables.md) vignette.
...
To demonstrate realistic speed gains, we compare the multiple-estimation call specified above with fitting the same sixteen formulas one by one using the `fixest` benchmark DGPs described in [When Are Fixed Effects Estimations Difficult?](../explanation/difficult-fixed-effects.md).
...
- [OLS with Fixed Effects](ols-fixed-effects.md): practical FE estimation patterns.
- [Difference-in-Differences](difference-in-differences.md): event-study applications with `i()`.
- [Regression Tables](regression-tables.md): organize and export many-model workflows.
- [How-To: Translating Stata to PyFixest](../how-to/stata-2-pyfixest.md): syntax mapping and defaults.
