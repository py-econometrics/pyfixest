<!-- Generated from docs/tutorials/difference-in-differences.qmd; do not edit. -->
...
These bands can be constructed by using a carefully chosen critical value $c$ that accounts for the covariance between coefficients using the multiplier bootstrap *(binary asset omitted)*. In pointwise inference, the critical value is $c = z_{1 - \alpha/2} = 1.96$ for $\alpha = 0.05$; the corresponding critical value for simultaneous inference is typically larger. These are also known as `sup-t` bands in the literature (see lec 3 of the NBER SI methods lectures linked above).
