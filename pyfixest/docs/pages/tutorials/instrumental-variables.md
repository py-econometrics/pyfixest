<!-- Generated from docs/tutorials/instrumental-variables.qmd; do not edit. -->
...
`PyFixest` provides two diagnostics for instrument strength: a standard first-stage F-statistic, and the more robust effective F-statistic from @oleapflueger2013 that remains valid under heteroskedasticity. Access them through the public `first_stage_model`, `first_stage_f_statistic`, and `effective_f_statistic` properties.

```python
# first_stage() must be called before IV_Diag() — it fits the first-stage OLS
# regression exposed as `first_stage_model`, which IV_Diag() requires.
fit_iv.first_stage()
fit_iv.first_stage_model.summary()
```

```python
fit_iv.IV_Diag()
print(f"First-stage F-statistic : {fit_iv.first_stage_f_statistic:.2f}")
print(f"Effective F-statistic   : {fit_iv.effective_f_statistic:.2f}")
```
...
fit_late.IV_Diag()
print(f"First-stage F-statistic : {fit_late.first_stage_f_statistic:.2f}")
print(f"Effective F-statistic   : {fit_late.effective_f_statistic:.2f}")
```
...
# The public property is available after calling first_stage().
print(f"First-stage F-statistic: {fit_iv.first_stage_f_statistic:.1f}")
...
print(f"Standard F-statistic:  {fit_iv.first_stage_f_statistic:.1f}")
print(f"Effective F-statistic: {fit_iv.effective_f_statistic:.1f}")
```

## Next Steps
- [Standard Errors & Inference](standard-errors.md) --- learn about robust, cluster-robust, and bootstrap inference.
- [Regression Tables](regression-tables.md) --- customize publication-ready output tables.
- [`Feiv` API Reference](../reference/estimation.models.feiv_.Feiv.md) --- full documentation of the IV estimator class.
