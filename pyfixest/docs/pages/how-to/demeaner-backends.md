<!-- Generated from docs/how-to/demeaner-backends.qmd; do not edit. -->
...
The `cupy` / `scipy` LSMR demeaner backends are deprecated and will be
removed in a future release. Switch to the recommendations above:

- Users running `LsmrDemeaner(backend="cupy", device="cuda")` on GPU should
  use `LsmrDemeaner(backend="torch", device="cuda")`.
