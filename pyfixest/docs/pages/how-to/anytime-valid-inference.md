...
The fixed-time analyses performed only on day twenty reject `{python} f"{fpr_planned_analysis:.1%}"` of the time, close to the intended 5%. Checking the fixed-time p-value daily and stopping at the first p-value below 0.05 inflates the rate to `{python} f"{fpr_daily_ordinary:.0%}"`. This is below the 64% independence benchmark; the twenty analyses reuse the same accumulating data and their p-values are strongly dependent. It is nevertheless several times the specified false-positive rate of 5%. SAVI rejects `{python} f"{fpr_daily_savi:.1%}"` of the same null experiments. This rate does not need to equal 5%: under the method’s assumptions, the probability of a false rejection at any time is asymptotically at most 5%, so the probability of rejecting by day twenty is also at most 5% and can be lower.
...
The daily fixed-time p-value stopping rule rejects in `{python} f"{ordinary_daily_rejection_rate:.0%}"` of the simulated experiments with null effects. Among the experiments that reject, the estimated effect at the stopping time averages `{python} f"{ordinary_daily_mean_abs:.1f}"` pp in absolute value even though the true effect is zero. These large magnitudes arise from selection at the stopping time.

The SAVI rule rejects `{python} f"{savi_rejection_rate:.1%}"` of these null experiments because its threshold is calibrated across all daily checks. Conditioning on a SAVI rejection still selects unusually large absolute point estimates.
...
The fixed-time p-value falls below 0.05 on day `{python} peek_day`, when the estimated lift is `{python} peek_lift`. But stopping at this point in time uses the event \\\min\_{t \leq 20} p_t \leq 0.05\\, for which the fixed-time p-values do not provide 5% type-I error control. Using fixed-time inference, you should not stop here, but only after 20 days.

With SAVI, the p-value falls below 0.05 on day `{python} savi_day`, after `{python} savi_users` users. The shipping rule in this example requires the SAVI confidence sequence to be positive and exclude zero. Because the lower end of the confidence sequence excludes zero before day twenty, the SAVI decision rule ships `{python} days_saved` days early. The confidence sequence at that day is `{python} savi_cs`. This establishes a positive effect, not an effect of at least 2 pp. If 2 pp were the minimum worthwhile lift rather than the planning MDE, shipping would require the lower endpoint to exceed 2 pp.
...
At day twenty, the fixed-time test detects the effect in `{python} f"{oneshot_power:.0%}"` of experiments when the true lift is the planning MDE of 2 pp. By then, SAVI has produced a positive rejection of the hypothesis of “no effect” in `{python} f"{larger_savi_detection[N_DAYS - 1]:.0%}"` of experiments with a 4 pp lift, `{python} f"{savi_detection[N_DAYS - 1]:.0%}"` with a 2 pp lift, and `{python} f"{smaller_savi_detection[N_DAYS - 1]:.0%}"` with a 1 pp lift. The same prespecified SAVI rule remains calibrated when monitoring continues past day twenty. By day forty, those shares are `{python} f"{larger_savi_detection[-1]:.0%}"`, `{python} f"{savi_detection[-1]:.0%}"`, and `{python} f"{smaller_savi_detection[-1]:.0%}"`, respectively. In this simulated design, SAVI has lower power than the fixed-time test at day twenty when the true lift equals the planning MDE of 2 pp. With a true 4 pp lift in the same design, SAVI often reaches its rejection boundary before day twenty. This is a stopping-time advantage, not a power advantage over the fixed-time test at the same effect and horizon.
...
At week 52, the displayed path has a SAVI confidence sequence of `{python} holdout_cs` and a fixed-time interval of `{python} holdout_ci`. To estimate simultaneous coverage, the analysis uses all `{python} HOLDOUT_SIMULATIONS` simulated holdouts and records whether each interval has contained the true lift at every review so far:
...
By week 52, the fixed-time interval has missed the true lift at least once in `{python} f"{1 - ci_always_covered[-1]:.0%}"` of the simulated holdouts. The SAVI confidence sequence did not miss in any of these 100 runs.
...
Initially, the SAVI confidence sequence is wide because one hour of data provides little information. After `{python} rollback_hour` hours, the SAVI confidence sequence is `{python} rollback_cs`; its upper endpoint is below -1 pp, so the rollback rule fires. A fixed-time test planned for hour 48 would instead have to wait another `{python} hours_saved` hours before making the same decision.
...
By day twenty, the median SAVI confidence-sequence half-width falls from `{python} f"{median_width['unadjusted'][-1]:.3f}"` without adjustment to `{python} f"{median_width['adjusted'][-1]:.3f}"` with adjustment. The adjusted analysis meets the stopping rule in `{python} f"{stopped_share['adjusted'][-1]:.0%}"` of experiments, compared with `{python} f"{stopped_share['unadjusted'][-1]:.0%}"` without the pre-experiment metric.

At day twenty, the average treatment estimates are `{python} f"{mean_estimate['unadjusted']:.3f}"` without adjustment and `{python} f"{mean_estimate['adjusted']:.3f}"` with adjustment, against a true average effect of `{python} f"{ADJUSTMENT_EFFECT:.3f}"`. Both are close to the true value in these 200 simulations; the adjusted analysis is more precise because the pre-experimental covariate predicts the experimental outcome.

To check whether adjustment inflates false positives in this design, the simulation is repeated with an average treatment effect of zero. Across `{python} ADJUSTMENT_NULL_SIMULATIONS` experiments, the SAVI p-value fell below 0.05 by day twenty in `{python} int(adjustment_null_rejections)` runs (`{python} f"{adjustment_null_rejection_rate:.1%}"`).
