...
def _model_plot_labels(
    models: Sequence[Feols | Fepois | Feiv],
    rename_models: dict[str, str] | None = None,
) -> list[str]:
    """
    Label each model for a plot legend or a tidy model frame.

    A model is labelled by its name, which is its formula, extended by the
    split variable and value under split estimation. Models sharing a name
    would overlap in a plot, so each of them is prefixed with its position
    among the duplicates instead.

    Parameters
    ----------
        models : Sequence[Union[Feols, Fepois, Feiv]]
                The models to label, as returned by
                `_post_processing_input_checks()`.
        rename_models : dict, optional
                The relabelling the caller will apply to the returned labels.
                Only checked for keys that match no model here; the caller
                applies it.

    Returns
    -------
        list[str]
            One label per model, in the order the models were given.
    """
    labels = [model._model_name for model in models]

    counter = Counter(labels)
    duplicate_model_names = [name for name, count in counter.items() if count > 1]

    for duplicate_model in duplicate_model_names:
        positions = [i for i, label in enumerate(labels) if label == duplicate_model]
        for i, position in enumerate(positions):
            labels[position] = f"Model {i}: {duplicate_model}"
            warnings.warn(
                f"The model name '{duplicate_model}' is duplicated for models in the `models` you provided. To avoid overlapping model names / plots, this model is labelled '{labels[position]}'."
            )

    if rename_models is not None:
        model_name_diff = set(rename_models.keys()) - set(counter)
        if model_name_diff:
            warnings.warn(
                f"""
