...
Alternatively, the `.summarize` module contains a `summary` function, which can be applied on instances of regression model objects or lists of regression model objects. For details on how to customize `etable()`, please take a look at the [dedicated vignette](tutorials/regression-tables.qmd).
...
For details, take a look at the dedicated [quantreg vignette](tutorials/quantile-regression.qmd).
...
For more details see the vignette on [Difference-in-Differences Estimation](tutorials/difference-in-differences.qmd).
