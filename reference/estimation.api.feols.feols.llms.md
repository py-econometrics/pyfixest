...
Returns an object of type [Feols](../reference/estimation.models.feols_.Feols.llms.md) or [Feiv](../reference/estimation.models.feiv_.Feiv.llms.md) (when using instrumental variables).
...
As in `fixest`, the [feols()](../reference/estimation.api.feols.feols.llms.md) function can be used to estimate a simple linear regression model with fixed effects. The following example regresses `Y` on `X1` and `X2` with fixed effects for `f1` and `f2`: fixed effects are specified after the `|` symbol.
...
Calling `feols()` returns an instance of the [Feols](../reference/estimation.models.feols_.Feols.llms.md) class. The `summary()` method can be used to print the results.
...
|-------------|-----------|------------|------------|-------------|-----------|-----------|
| Coefficient |           |            |            |             |           |           |
| X1          | -0.924046 | 0.056060   | -16.483063 | 0.0         | -1.034065 | -0.814028 |
| X2          | -0.174107 | 0.014859   | -11.717062 | 0.0         | -0.203269 | -0.144946 |

You can also access common outputs via dedicated methods, e.g. `fit.coef()` for the coefficients, `fit.se()` for the standard errors, `fit.tstat()` for the t-statistics, `fit.pvalue()` for the p-values, and `fit.confint()` for the confidence intervals.
...
The `ssc` argument specifies the small sample correction for inference. In general, `feols()` uses all of `fixest::feols()` defaults, but sets the `fixef.K` argument to `"none"` whereas the `fixest::feols()` default is `"nested"`. See here for more details: [link to github](https://github.com/py-econometrics/pyfixest/issues/260).
...
Besides OLS, `feols()` also supports IV estimation via three-part formulas. IV models return an instance of the [Feiv](../reference/estimation.models.feiv_.Feiv.llms.md) class (which inherits from [Feols](../reference/estimation.models.feols_.Feols.llms.md)).
...
```
    ...
    pyfixest.estimation.models.feiv_.Feiv
...
To diagnose weak instruments, use the `IV_Diag()` method, which computes the first-stage F-statistic and the effective F-statistic (Olea and Pflueger, 2013):
...
```
    ...
    First-stage F-statistic: 620.086
    Effective F-statistic: 962.93

You can also access the first-stage regression as a `Feols` object via `_model_1st_stage` and display both stages with `etable()`:
...
Last, `feols()` supports interaction of variables via the `i()` syntax. For a compact overview of formula features including `i()`, see the [formula syntax tutorial](../tutorials/formula-syntax.llms.md).
...
```
    ...
    array([ 3.06336629, -0.69574135, -0.91240434, -0.46370257, -1.67331151])
...
|      | Estimate             | Std. Error | t value  | Pr(\>\|t\|) | 2.5%      | 97.5%    |
|------|----------------------|------------|----------|-------------|-----------|----------|
| CCV  | 0.016087657906364097 | 0.284344   | 0.056578 | 0.955505    | -0.581297 | 0.613472 |
