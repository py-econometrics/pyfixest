...
- Never hand-edit `docs/reference/**` — it is generated by quartodoc and
  gitignored.
- For every public API, behavior, error, or workflow change, update the
  canonical docstring or authored source page first. If it affects agent routing,
  update `ARCHITECTURE.md` and/or `skills/pyfixest/` too, then regenerate the
  tracked package corpus with
  `pixi run -e py312-r python docs/_scripts/generate_llms_md.py --no-site`.
  Never hand-edit `pyfixest/docs/**`, `docs/skills.md`, or `docs/llms.txt`:
...
pixi run -e py312-r python docs/_scripts/generate_llms_md.py --check --no-site
pixi run -e py312-r python scripts/check_skill.py skills/pyfixest
