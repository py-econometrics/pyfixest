...
**Rebuilds after `.rs` changes:** The editable install does not detect Rust
source changes. After modifying `.rs` files, rebuild manually:
...
Several R test tasks use pixi's `depends-on`, `inputs`, and `outputs` to
...
|---|---|---|
| `maturin-develop` | default | Rebuild Rust extension after `.rs` changes |
