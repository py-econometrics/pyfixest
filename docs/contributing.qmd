...
- Install all Python, Rust, and R dependencies via pixi
- Compile the Rust extension
- Set up pre-commit hooks

Once ready (~7-10 minutes), you can immediately start working:

```{.bash .code-copy}
# Run the quick test suite
pixi run tests-regular

# Run all tests including R comparison tests
pixi run tests

# Run linting
pixi run -e lint pre-commit run --all-files

# Activate the dev environment shell
pixi shell -e dev
```

The codespace includes VSCode extensions for Python, Ruff, Rust, and Jupyter pre-configured.
