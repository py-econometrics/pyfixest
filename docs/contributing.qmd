...
To install `pixi`, follow the [installation instructions](https://pixi.sh/latest/#installation). `PyFixest` needs pixi 0.71.0 or newer: the workspace declares its CUDA requirement as a [rich platform](https://pixi.sh/latest/workspace/multi_platform_configuration/) and the lockfile is format v7. Environments are installed automatically when you run a task (e.g. `pixi run test-py`). You do not need a global Rust installation -- Rust is installed via conda.
