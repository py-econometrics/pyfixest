...
Categorical variables are automatically be relabeled from the formulaic "C(variable)[T.value]" notation where the default is *{variable}={value}*. This works when either the variable is categorial in the DataFrame, or the C() or i() operators are used in the regresson formula. If you instead want to keep the standard formulaic notation, specify *cat_template=""*.
´
```{python}
