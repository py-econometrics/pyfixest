...
convergence. `LsmrDemeaner` uses `fixef_atol` and `fixef_btol`; PyFixest passes
the larger of the two tolerances to the `within` and torch LSMR solvers for
compatibility with the older SciPy-style API.
