...
| Backend | When to use |
|---|---|
| `MapDemeaner(backend="rust")` | Default. Works well for dense fixed effects / well connected graphs and avoids the optional Numba dependency. |
| `MapDemeaner(backend="numba")` | Optional MAP implementation. Requires installing `pyfixest[numba]`. |
