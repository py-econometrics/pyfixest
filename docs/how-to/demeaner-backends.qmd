...
factors. For single fixed effects, PyFixest uses the MAP fallback and no
preconditioner is computed or applied. For large multi-way FE problems this
*setup* phase can be a meaningful fraction of total runtime, so it is worth
reusing the preconditioner across solves on the same design.

### Within a session

Every fit produced with `LsmrDemeaner(backend="within")` exposes its built
preconditioner via the `preconditioners` attribute:
...
`WithinPreconditioner` instances are pickleable, so the setup phase can be
...
with open(tmp, "wb") as f:
    pickle.dump(fit.preconditioners[0], f)
