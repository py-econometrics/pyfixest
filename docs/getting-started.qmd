...
description: "Install PyFixest and fit your first fixed effects regression, estimating the causal returns to education from a twin study."
aliases:
  - /quickstart.html
format:
  html:
...
Common model-level fit statistics are stored on the fitted object; the
`fitstat` component holds the goodness-of-fit measures:

```{python}
fit_naive.sample_info.n_obs  # number of observations
fit_naive.fitstat.rmse       # root mean squared error
fit_naive.fitstat.r2         # R-squared
fit_naive.fitstat.adj_r2     # adjusted R-squared
```

For models with fixed effects, within R-squared measures are also available:

```{python}
fit_fe.fitstat.r2_within
fit_fe.fitstat.adj_r2_within
```
