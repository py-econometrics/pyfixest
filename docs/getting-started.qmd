...
```{python}
fit_naive.sample_info.n_obs  # number of observations
...
We can quantify how strong omitted confounding would need to be to explain away
the naive estimate. Sensitivity analysis is configured for one named treatment;
here we benchmark an omitted confounder against observed experience:

```{python}
sensitivity = fit_naive.sensitivity_analysis("educ")
sensitivity.sensitivity_stats()
sensitivity.ovb_bounds("experience", kd=[0.5, 1], ky=[0.5, 1])
```

Because `fit_naive` was fitted with clustered standard errors, this call warns
that sensitivity analysis uses IID standard errors and residual degrees of
freedom. That convention follows the reference `sensemakr` implementation; it
does not replace the clustered inference reported for the fitted model. See the
[omitted-variable-bias sensitivity guide](how-to/omitted-variable-bias.qmd) for
interpretation and supported model configurations.
