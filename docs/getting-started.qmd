...
Use `summary()` for a model-level overview that includes fit statistics, or
`etable()` when comparing models:

```{python}
fit_naive.summary()
pf.etable([fit_naive, fit_fe])
```

Underscore-prefixed attributes hold implementation state and are not part of
the public API. Prefer the public result methods and reporting functions above
so code remains stable across PyFixest releases.
