...
```{python}
fit_weights.covariance.vcov - stats.vcov(r_fit_weights)
```
...
```{python}
fit.covariance.vcov - stats.vcov(r_fit)
```
...
```{python}
fit_iid.covariance.vcov - stats.vcov(fit_r_iid)
```

```{python}
fit_hetero.covariance.vcov - stats.vcov(fit_r_hetero)
```

```{python}
fit_crv.covariance.vcov - stats.vcov(fit_r_crv)
```
