...
fixed-effects estimation is a **graph problem**: the structure of who-works-where (or who-sees-which-doctor which-brand-in-which-store) determines how hard the problem is. After reading this guide, you should (hopefully) have a good idea if you can speed up your own fixed effects problem by choosing a different solver.
...
Why does convergence depend on the graph? Consider what happens in the worker-mean step. Each worker's mean is computed over all firms they have worked at. When a worker has moved across several firms, their mean averages over different firm effects, which partially cancel out. Subtracting this mean removes most of the worker component and leaves a residual that is informative about firms. The subsequent firm-mean step then makes clean progress, and so on.

When the graph is sparse (few movers), workers are essentially **nested** within firms — nearly every worker is observed at a single firm only. A worker's mean is now the average outcome at just one firm, which confounds the worker effect with that firm's effect. Subtracting it removes both at once, leaving a residual that carries almost no signal about either. The algorithm must iterate many times, slowly propagating the little cross-factor information that exists through the few movers that bridge different firms.
