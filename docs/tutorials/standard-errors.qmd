...
reweights observations directly: each iteration draws a new weight vector,
re-fits the model under those weights, and collects the resulting
coefficients. Standard errors and confidence intervals come from the
resulting bootstrap distribution.

Two weight-drawing schemes are available:

- `method="bayesian"` — weights drawn from a Dirichlet distribution
  (the Bayesian bootstrap of Rubin, 1981). Continuous weights, no ties.
- `method="multinomial"` — weights drawn as integer counts from a
  Multinomial distribution. Equivalent to the classical pairs/XY bootstrap
  (resampling rows with replacement).

```python
wb_res = fit_crv1.weightingboottest(reps=999, method="bayesian", cluster="group_id")
wb_res
```
...
`cluster` column explicitly.

The weighting bootstrap also supports IV (2SLS) and GLM (Poisson, logit,
probit) models, where the wild cluster bootstrap does not:

| | Wild cluster bootstrap | Weighting bootstrap |
|---|---|---|
| Perturbs | residuals | observation weights |
| IV (2SLS) | not supported | supported |
| WLS | not supported | supported |
| Multiway clustering | not supported | not supported |
| Best suited for | few/uneven clusters, OLS | IV, GLM, or as a wild-bootstrap alternative |
...
3. If clusters are few or very uneven, use wild cluster bootstrap (OLS) or the weighting bootstrap (IV, GLM, or as an alternative).
