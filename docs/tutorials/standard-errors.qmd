...
- `bayesian_bootstrap()`: continuous Dirichlet weights give the posterior draws
  ...
  reports posterior SD, an equal-tail credible interval, and a two-sided
  posterior tail probability for zero. `alpha=1` is Rubin's standard choice;
  other finite positive values change the common Dirichlet concentration.
- `pairs_bootstrap()`: integer counts implement a pairs bootstrap while the
  ...
  and the finite-replication-corrected p-value `(1 + exceedances) / (reps + 1)`
  for an unstudentized centered test of `H0: beta = 0`.

```python
fit_weighting = pf.feols("Y ~ X1 + X2 | f1", data=data, vcov={"CRV1": "f1"})
bayesian_result = fit_weighting.bayesian_bootstrap(
    reps=999,
    alpha=1.0,
    cluster="f1",
    seed=42,
)
pairs_result = fit_weighting.pairs_bootstrap(
    reps=999,
    cluster="f1",
    seed=42,
)
...
offsets are also unsupported. The method needs the original model matrices and
data, so fit with `lean=False` and `store_data=True`.
...
every retained draw. GLM draws always combine bootstrap weights with the
original user weights, not final IRLS working weights. A draw is discarded if
estimator convergence, finite-value, rank, or IV identification checks fail.
For binary GLMs, draws with one outcome class or a boundary solution are also
discarded; this replicate guard does not rerun the model's configured
`separation_check` procedure.
The attempt count is bounded; a warning reports failures and states that the
returned inference is conditional on the requested number of successful draws.
...
`Posterior tail probability`, and `interval`. Pairs output contains `Estimate`,
`CI lower`, `CI upper`, `Bootstrap SE`, `P-value`, and `interval`.
