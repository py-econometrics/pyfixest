...
description: "TWFE, Gardner's two-stage DID2S, local projections, and event study designs with heterogeneous treatment effects."
...
fit_dynamic_twfe.iplot(
    ...
    yintercept=0,
    xintercept=13.5,
    labels=rename_event_study_coefs(fit_dynamic_twfe.coefnames),
)
```
...
fit_twfe.iplot(
    ...
    xintercept=18.5,
    yintercept=0,
    labels=rename_event_study_coefs(fit_twfe.coefnames),  # rename coefficients
).show()
```
