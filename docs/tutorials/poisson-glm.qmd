---
title: "Poisson & GLMs"
description: "Poisson for nonnegative outcomes, DiD with counts, and logit/probit for propensity-score based AB testing."
...
def make_trade_data(seed=123, n_exporters=15, n_importers=15, years=range(2010, 2016)):
    ...
    rows = []
    for y in years:
        for exporter_index, e in enumerate(exporters):
            for importer_index, i in enumerate(importers):
                dist = rng.uniform(200, 9000)
                fta = rng.binomial(1, 0.15)
                exporter_fe = exporter_index % 7
                importer_fe = importer_index % 6
                year_fe = y - min(years)
