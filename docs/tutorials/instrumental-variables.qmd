...
`PyFixest` provides two diagnostics for instrument strength: a standard first-stage F-statistic, and the more robust effective F-statistic from @oleapflueger2013 that remains valid under heteroskedasticity. Both live on the `.first_stage` attribute, which every IV fit carries.

```{python}
# The first-stage OLS regression is fitted with the model and retained as
# fit_iv.first_stage.model.
fit_iv.first_stage.model.summary()
```

```{python}
# IV_Diag() adds the effective F-statistic to fit_iv.first_stage.diagnostics.
fit_iv.IV_Diag()
diagnostics = fit_iv.first_stage.diagnostics
print(f"First-stage F-statistic : {diagnostics.f_stat:.2f}")
print(f"Effective F-statistic   : {diagnostics.eff_f:.2f}")
```
...
```{python}
fit_late.IV_Diag()
diagnostics = fit_late.first_stage.diagnostics
print(f"First-stage F-statistic : {diagnostics.f_stat:.2f}")
print(f"Effective F-statistic   : {diagnostics.eff_f:.2f}")
```
...
Every IV fit estimates its first stage and reports the **first-stage F-statistic**, which tests $H_0\colon \pi = 0$ (all instrument coefficients are jointly zero). The classic rule of thumb is $F > 10$ for iid errors (@stockyogo2005).


```{python}
# The first stage inherits the model's covariance estimator, and IV_Diag()
# switches the first stage to hetero for the effective F computation. Refit
# with iid errors to read the iid-based first-stage F-statistic.
fit_iid = pf.feols("earnings ~ 1 | num_children ~ ivf_success", data=ivf_df, vcov="iid")

diagnostics = fit_iid.first_stage.diagnostics
print(f"First-stage F-statistic: {diagnostics.f_stat:.1f}")
print(f"First-stage p-value:     {diagnostics.p_value:.4f}")
```
...
```{python}
# Re-use the iid fit above: IV_Diag() reports the standard F under the first
# stage's own covariance estimator before switching it to hetero for the
# effective F.
fit_iid.IV_Diag()

diagnostics = fit_iid.first_stage.diagnostics
print(f"Standard F-statistic:  {diagnostics.f_stat:.1f}")
print(f"Effective F-statistic: {diagnostics.eff_f:.1f}")
```
