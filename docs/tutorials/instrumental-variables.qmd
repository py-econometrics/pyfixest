...
```{python}
# Re-use the motherhood penalty IV model. IV_Diag() now preserves the retained
# first-stage covariance, so the iid-based F-statistic remains available.
fit_iv.IV_Diag()

# The F-stat is stored as an attribute after calling IV_Diag()
