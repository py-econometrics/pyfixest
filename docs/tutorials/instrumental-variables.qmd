...
```{python}
# Re-use the motherhood penalty IV model. IV_Diag() preserves the retained
# first-stage's own covariance, so the iid-based F-statistic remains available.
fit_iv.IV_Diag()
...
```{python}
# Re-use the motherhood penalty IV model: IV_Diag() reports the standard F
# under the first stage's own covariance estimator, and computes the
# effective F under a heteroskedasticity-robust covariance without changing
# the first stage's retained covariance.
fit_iv.IV_Diag()
