...
aliases:
  - /table-layout.html
description: "Publication-ready tables with Great Tables or LaTeX booktabs — customize labels, significance stars, and output formats."
categories: [Workflow]
format:
  html:
...
The `pf.etable()` API remains unchanged and `pf.make_table()` has been removed (use `maketables.MTable()` directly).
:::
...
Notice that this process will change the `coefnames`. In this example, the new `coefnames` are:
```{python}
fit9.coefnames
```
