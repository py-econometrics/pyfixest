...
Supported covariance types are "iid", "HC1-3", CRV1 and CRV3 (up to two-way clustering), NW/DK HAC, and Conley spatial HAC.
...
- `NW`, `DK`, and `conley`: HAC-style standard errors. `NW` and `DK` use `vcov_kwargs` with `time_id`, while `conley` uses `vcov_kwargs` with `lat`, `lon`, and `cutoff`. For Conley, a zero cutoff includes only observations with identical coordinates.
