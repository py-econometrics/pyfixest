...
| [**FixedEffects.jl**](https://github.com/FixedEffects/FixedEffects.jl) | Julia | Matthieu Gomez's Julia package for high-dimensional fixed effects. PyFixest's variance-ratio collinearity check for LSMR backends (`collin_tol_var`) and the default LSMR convergence tolerance (`1e-8`) are informed by the defaults and approach in FixedEffects.jl. |
