...
In both `fixest` and `pyfixest`, small sample corrections are controlled by the `ssc` function. In `pyfixest`, `ssc` accepts four arguments: `adj`, `cluster_adj`, `k_adj` and `cluster_df`.
...
- **`k_adj`**: Determines how `df_k` is computed (how fixed effects are counted).
...
The `k_adj` argument controls how fixed effects contribute to `df_k`, and thus to `adj_val`. It supports three options:
...
Fixed effects may be **nested** within cluster variables (e.g., district FEs nested in state clusters). If `k_adj = "nonnested"`, nested fixed effects do not count toward `k_fe`:
