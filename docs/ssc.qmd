...
In both `fixest` and `pyfixest`, small sample corrections are controlled by the `ssc` function. In `pyfixest`, `ssc` accepts four arguments: `adj`, `G_adj`, `k_fixef` and `G_df`.
...
- **`G_adj`**: Enables or disables the second scalar adjustment.
