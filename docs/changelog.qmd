...
- Fitted models expose `fit.coefnames`, `fit.k`, and `fit.X_is_empty` as read-only properties derived from `fit.collinearity`: the names of the estimated coefficients after the rank check, their number, and whether the fit estimates none (a fixed-effects-only fit). They replace the private attributes `_coefnames`, `_k`, and `_X_is_empty`, and survive `store_data=False` and `lean=True`.
