...
- Fitted OLS, fixed-effect OLS, IV, and Gaussian, Poisson, logit, and probit GLMs
  now provide `bayesian_bootstrap()` and `pairs_bootstrap()` methods. The methods
...
- Bayesian output reports the original estimate, posterior mean, posterior SD,
  equal-tail credible limits, and posterior tail probability. Pairs output
  reports the original estimate, bootstrap SE, percentile confidence limits,
  and an unstudentized centered p-value.
- Frequency weights, quantile regression, offsets, multiway clustering, and
  models fit with `lean=True` or `store_data=False` are not supported. Weighting
  bootstraps require many independent observations or clusters and are not a
  few-cluster replacement for the wild cluster bootstrap.

```{python}
bootstrap_fit = pf.feols("Y ~ X1 + X2 | f1", data=df)
bootstrap_fit.bayesian_bootstrap(reps=19, seed=42)
bootstrap_fit.pairs_bootstrap(reps=19, seed=42)
```
