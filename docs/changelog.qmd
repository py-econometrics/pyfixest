...
- Fitted models expose `fit.sample`, a `SampleInfo` component with the positions of the retained and excluded input rows, the physical and effective (frequency-weight) observation counts, and `ExclusionCounts` by filtering stage (missing, nonfinite, singleton, separation). It replaces the private `_N`, `_N_rows`, and `_na_index` fields and the public `n_separation_na` attribute of fitted models, and the count fields of `ObservationWeights`. `lean=True` drops only the retained positions.
