...
- Apply storage options recursively to components and IV first stages, release execution caches, and raise `MissingModelDataError` when an operation needs omitted state.
- Publish estimator components for inspection and remove superseded private array aliases; see [fitted-state access](/how-to/fitted-state.qmd).
- Complete Gaussian performance before storage cleanup, and retain usable predictions for both multi-quantile methods.
