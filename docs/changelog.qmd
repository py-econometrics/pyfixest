...
- `vcov()` publishes `fit.variance_covariance`, a `VarianceCovariance` value with the adjusted `vcov`, the `meat` where a sandwich exists (so that `vcov == bread @ meat @ bread`), the small-sample factors `ssc`, `df_k`, `df_t`, the requested estimator as a `VcovSpec` (`spec.vcov_type`, `spec.vcov_type_detail`, `spec.clustervar`, and the HAC `lag`, `time_id`, `panel_id`), and the cluster counts `G`. Malformed `vcov` input now raises `ValueError` naming the allowed values instead of `AssertionError`, and `did2s` fits record their cluster variable, so `wald_test()` uses the clustered degrees of freedom. It replaces the private attributes `_vcov`, `_vcov_type`, `_vcov_type_detail`, `_is_clustered`, `_clustervar`, `_G`, `_ssc`, `_df_k`, `_df_t`, and `_cluster_df`.
