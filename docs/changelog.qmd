...
- `store_data=False` and `lean=True` now apply to multiple-estimation
  containers; default multi-quantile results retain the arrays needed for
  prediction and covariance updates.
...
- For weighted models, `_sumFE` was stored as `sqrt(w) * D @ alpha`. It is now
  `D @ alpha`, on the scale of the dependent variable, matching fixest's
