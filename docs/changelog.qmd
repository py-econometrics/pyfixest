...
- Permanent R references now compare frequency-weighted Gaussian, logit, and
  probit GLMs with literal row expansion across fixed effects, covariance
  estimators, small-sample corrections, and separation-filtered samples.
...
- Fitted results are now built from a shared `BaseRegression` class and one leaf
  per estimator. `Feols` is the OLS leaf, `Feiv` extends it, and `Feglm` and
  `Quantreg` are separate leaves rather than `Feols` subclasses. Estimator
  differences inside a shared method are hooks on the base instead of
  comparisons against the mutable `_method` label. Reporting and the result
...
- Rejected combinations now share one message, `"<feature> is not supported for
  <reason>."`. `CRV3` raises `VcovTypeNotSupportedError`, and every feature a
  fit could support raises `NotImplementedError`.
- **Breaking:** post-estimation methods an estimator never supports are no
  longer defined on its result class. `wildboottest()`, `ccv()`, `decompose()`,
  `update()`, `evalue()`, and `pvalue_savi()` are OLS-only, so a Poisson, GLM,
  or quantile result raises `AttributeError` instead of `NotImplementedError`,
  and `hasattr(fit, "decompose")` is now `False` for those fits. The message
  still carries the reason from the capability table and points at
  `capabilities()`. Fit-dependent restrictions -- weights, fixed effects, IV --
  keep raising `NotImplementedError` from the method itself.
- `quantreg()` results now reject `predict(se_fit=True)` and
  ...
  instead.
- `feglm()` and `fepois()` results now reject `predict(interval="prediction")`
  as they already rejected `predict(se_fit=True)`. The GLM prediction overrides
  did not forward `interval`, so the argument was silently dropped and point
  predictions were returned in place of a prediction interval.
