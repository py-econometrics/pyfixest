...
- Executed documentation examples now use fixed seeds and index-based fixed
  effects instead of `hash()`, and every figure carries alt text.
...
![](explanation/figures/base-benchmarks/bench_difficult.png){width=85% fig-align="center" fig-alt="Sparse fixed-effects benchmark comparing MAP with within LSMR."}
...
data["w"] = np.random.default_rng(123).random(500)
...
- Adds support for simultaneous confidence intervals via a multiplier bootstrap. Thanks to [@apoorvalal](https://github.com/apoorvalal) for the contribution!
  ...
  ```{python}
  fit.confint(joint = True, seed = 123)
  ```
...
- Adds support for the causal cluster variance estimator by [Abadie et al. (QJE, 2023)](https://academic.oup.com/qje/article/138/1/1/6750017)
  ...
  ```{python}
  fit.ccv(treatment = "D", cluster = "group_id", seed = 123)
  ```
