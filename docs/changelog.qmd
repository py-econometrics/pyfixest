...
- `event_study()` and `did2s()` publish `fit.did_design`, a `DidDesign` value with the `estimator` (`"did2s"`, `"twfe"`, or `"saturated"`), the outcome, unit, period and cohort variable names, the covariate formula, the `att` flag, and the `cluster` variable; `EventStudyDesign` extends it with the cohort curves. The wrappers no longer overwrite `_method`, which names the estimation function a fit is refitted with, so a CRV3 jackknife of an event study is no longer refitted with Poisson, and `summary()` of a saturated event study reports its estimator instead of raising `ValueError`.
- `coefplot()`, `iplot()`, and `qplot()` compute their model labels from the models they are given instead of writing them back as `_model_name_plot`, which is removed. The labels, and with them the keys of `rename_models`, are unchanged.
