...
The table functionality in pyfixest now uses the [maketables](https://py-econometrics.github.io/maketables/) package internally.

**Changes:**

- `pf.etable()` now uses `maketables.ETable` internally. The API remains unchanged for backward compatibility.
- `pf.dtable()` is deprecated. A `FutureWarning` is now emitted. Use `maketables.DTable()` directly.
- `pf.make_table()` has been removed. Use `maketables.MTable()` directly.
- The `show_se_type` parameter has been removed from `etable()`.
