...
- The estimation functions parse `vcov` and `vcov_kwargs` before fitting, so a misspelled estimator such as `vcov="hc1"`, a cluster, time, or panel variable missing from the data (now a `ValueError` rather than a `KeyError`), or an estimator the model does not support, such as `"HC2"` with fixed effects, fails before any model is estimated. `vcov_kwargs` follows one set of rules in the estimation functions and in `vcov()`: unknown keys, a `lag` that is not a non-negative integer, and non-string `time_id` or `panel_id` values raise `ValueError` whatever the requested estimator. `VcovSpec.from_user_input()` no longer takes `has_fixef` and `is_iv`, and `VcovSpec.vcov_type` is typed as a `Literal`.
