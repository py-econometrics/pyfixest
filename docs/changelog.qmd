...
`MapDemeaner()` now defaults to `backend="rust"`. The previous Numba MAP backend
is deprecated but still available via `MapDemeaner(backend="numba")`, and fast
randomization inference still uses Numba. Numba is now an optional dependency
installed with `pip install pyfixest[numba]`.
...
*Benchmark on a sparse fixed effects structure: vanilla MAP degrades dramatically, while LSMR-Schwarz via `pf.LsmrDemeaner(backend="within")` remains fast.*
