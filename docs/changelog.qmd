...
- `iplot()` now selects `i()` coefficients from the formula's term registry instead of matching `::` in coefficient names, so a regressor whose column name contains `::` is no longer plotted as an interaction. The private `Feols._icovars` attribute was removed.
