...
- The rendered `llms.txt` documentation now has deterministic link and retrieval
  checks for common agent questions.
- The package, README, and contributor guide now direct AI agents to the
  machine-readable documentation and repository instructions.
- Release artifacts now carry a deterministic, version-matched narrative
  documentation corpus generated from Quarto's `.llms.md` output.
- A canonical PyFixest skill now gives AI agents a short entry point into the
  bundled docs, with focused references for common analysis workflows.
- The Standard Errors tutorial now documents robust, clustered, HAC, bootstrap,
  and randomization-based inference with executable examples and support limits.
