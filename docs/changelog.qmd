...
- Multiple estimations reuse immutable blocks of demeaned columns, avoid copying the growing cache on every new regressor, and share repeated design selections across outcomes.

- `decompose()` now raises `ValueError` on a fit with multiway clustering instead of silently bootstrapping on the first cluster variable only; pass a single `cluster` to choose the bootstrap dimension.
- Removed the deprecated `FixestFormulaParser` class and `model_matrix_fixest()` function (deprecated since 0.50.0; use `Formula.parse()` and `create_model_matrix()`), together with the `pyfixest.estimation.deprecated` package and the `pyfixest.estimation.feols_`, `feiv_`, and `fepois_` import shims. Import `Feols`, `Feiv`, and `Fepois` from `pyfixest` or `pyfixest.estimation.models`.
- IV diagnostics now compute the heteroskedasticity-robust effective F-statistic without replacing the retained first-stage covariance, so its original F-statistic and inference remain available.
- Post-estimation methods now raise `MissingModelDataError` when `store_data=False` or `lean=True` removed required state, and these storage options also clean retained IV first-stage models.
- Both multi-quantile methods now retain each child's design and response until `lean=True` cleanup, matching OLS and enabling child predictions and covariance updates.
- GLM residuals are no longer retained with `lean=True`, matching OLS cleanup.
- Fitted models expose `fit.sample_info`, an `EstimationSample` component with the number of observations `n_obs` (the frequency-weight sum, as fixest's `nobs`), the physical row count, the positions of the dropped input rows, and `DroppedRowCounts` by filtering stage (missing, infinite, singleton, separation). It replaces the private `_N`, `_N_rows`, and `_na_index` fields and the public `n_separation_na` attribute of fitted models, and the count fields of `ObservationWeights`; `ModelMatrix.na_index` is now `dropped_row_index`. The sample survives `store_data=False` and `lean=True`.
- Fitted models expose `fit.fitstat`, a `FitStatistics` component with the RMSE, R², adjusted and within R², deviance, and the Poisson log-likelihood, null log-likelihood, pseudo R², and Pearson chi² measures; a measure the estimator does not define is `NaN`. It replaces the private `_rmse`, `_r2`, `_adj_r2`, `_r2_within`, `_adj_r2_within`, `_loglik`, `_loglik_null`, `_pseudo_r2`, and `_pearson_chi2` fields, the public `deviance` attribute, and `get_performance()`. Gaussian `feglm()` fits now report their R² measures at fit time and in `summary()`.
- OLS, IV, and GLM fits expose `fit.sandwich`, a `SandwichComponents` value with the weighted `scores`, the `hessian`, and `bread`. IV fits build it from the first-stage projection of the design, as fixest does, so the 2SLS sandwich takes the OLS form. It replaces the private attributes `_scores`, `_hessian`, `_bread`, `_tZX`, `_tXZ`, `_tZy`, and `_tZZinv`, and is dropped by `lean=True`.
- `pf.ssc()` returns a frozen `Ssc` value instead of a dict; the estimation functions still accept a legacy dict. `get_ssc()` takes an `Ssc` and a `DegreesOfFreedomCounts` and returns a `SmallSampleCorrection` (`adj`, `df_k`, `df_t`); invalid `k_fixef` or `G_df` values now raise `ValueError` naming the allowed values.
- Saturated event studies publish `fit.event_study_design`, an `EventStudyDesign` value with the outcome, unit, period, and cohort variable names, the `att` flag, and the cohort-specific event-time curves. It replaces the private `_res_cohort_eventtime_dict`, `_yname`, `_gname`, `_tname`, `_idname`, and `_att` attributes that every fitted model used to carry, so `aggregate()`, `iplot_aggregate()`, and `test_treatment_heterogeneity()` now raise `AttributeError` instead of `NotImplementedError` on models that are not saturated event studies.
- The estimation functions parse `vcov` and `vcov_kwargs` before fitting, so a misspelled estimator such as `vcov="hc1"`, a cluster, time, or panel variable missing from the data (now a `ValueError` rather than a `KeyError`), or an estimator the model does not support, such as `"HC2"` with fixed effects, fails before any model is estimated. `vcov_kwargs` follows one set of rules in the estimation functions and in `vcov()`: unknown keys, a `lag` that is not a non-negative integer, and non-string `time_id` or `panel_id` values raise `ValueError` whatever the requested estimator. A `time_id` or `panel_id` that the requested estimator does not use is no longer checked against the data. `VcovSpec.from_user_input()` no longer takes `has_fixef` and `is_iv`, and `VcovSpec.vcov_type` is typed as a `Literal`.
- `get_inference()` publishes `fit.coeftable`, a `CoefficientTable` value with the `estimate`, `se`, `tstat`, `pvalue`, and `conf_int` arrays and the `alpha` of the bounds, mirroring fixest's `coeftable()`. It replaces the private attributes `_se`, `_tstat`, `_pvalue`, and `_conf_int`.
- `vcov()` publishes `fit.variance_covariance`, a `VarianceCovariance` value with the adjusted `vcov`, the `meat` where a sandwich exists (so that `vcov == bread @ meat @ bread`), the small-sample factors `ssc`, `df_k`, `df_t`, the requested estimator as a `VcovSpec` (`spec.vcov_type`, `spec.vcov_type_detail`, `spec.clustervar`, and the HAC `lag`, `time_id`, `panel_id`), and the cluster counts `G`. Malformed `vcov` input now raises `ValueError` naming the allowed values instead of `AssertionError`; a `vcov` dict with more than one key, unknown `vcov_kwargs` keys in post-estimation `vcov()` calls, and cluster variables written as an interaction such as `"f1^f2"` are rejected as well. `did2s` fits record their cluster variable, so `wald_test`, `wildboottest`, and `ccv` treat them as clustered. It replaces the private attributes `_vcov`, `_vcov_type`, `_vcov_type_detail`, `_is_clustered`, `_clustervar`, `_G`, `_ssc`, `_df_k`, `_df_t`, and `_cluster_df`.
- IV fits expose `fit.first_stage`, a `FirstStage` value with the first-stage `coefficients`, `fitted_values`, `residuals`, the fitted first-stage `model`, the excluded `instruments`, and `FirstStageDiagnostics` (`f_stat`, `p_value`, and `eff_f` once `IV_Diag()` or `eff_F()` runs). It replaces the private attributes `_pi_hat`, `_X_hat`, `_v_hat`, `_model_1st_stage`, `_non_exo_instruments`, `_p_iv`, `_iv_loc`, `_f_stat_1st_stage`, `_p_value_1st_stage`, and `_eff_F`. The `first_stage()` method, which runs automatically after every IV fit, is now the private `_fit_first_stage()`. The excluded instruments are ordered deterministically, so the first-stage F and effective F no longer vary in their last digits between sessions with several instruments.
- `wald_test()` now returns a `WaldTest` value, which it also publishes as `fit.wald`, with the statistic `stat` of the reference distribution used, its `pvalue`, `df1`, `df2`, the `distribution`, the `vcov_type`, and both scalings of the quadratic form (`wald_statistic` and `f_statistic`). It replaces the returned `pd.Series` and the private attributes `_dfn`, `_dfd`, `_wald_statistic`, `_f_statistic`, and `_p_value`. `test_treatment_heterogeneity()` returns the same value.
- `fixef()` publishes `fit.fixef_estimates`, a `FixedEffectEstimates` value with the `FixedEffect` coefficient records, the dummy-coded solution `alpha`, and the per-observation fixed-effect contribution `sumFE`. It replaces the private attributes `_fixef_coefficients`, `_alpha`, and `_sumFE`, and is absent until `fixef()` runs.
- Fitted models expose `fit.fitted_values`, a `FittedValues` value with the in-sample predictions on the `link` and the `response` scale, as fixest's `fitted.values(type=)` distinguishes them. For OLS, IV, and quantile fits the two scales coincide; GLMs publish the linear predictor as `link` and its inverse-link mean as `response`. It replaces the private attributes `_Y_hat_link` and `_Y_hat_response`, and is dropped by `lean=True`.
- `ritest(store_ritest_statistics=True)` publishes `fit.ritest_statistics`, a `RitestStatistics` value with the resampled `statistics`, the centered `sample_stat`, and the `pvalue`. It replaces the private attributes `_ritest_statistics`, `_ritest_sample_stat`, and `_ritest_pvalue`.
- Fitted models publish the rank check that removes collinear columns: `fit.collinearity`, a `CollinearityCheck` value with the `dropped_coef_names` (fixest's `collin.var`), the column `mask`, and the retained `coefnames`, plus `fit.collinearity_instruments` for the IV instrument matrix. It replaces the private attributes `_collin_vars`, `_collin_index`, `_collin_vars_z`, and `_collin_index_z`, and survives `store_data=False` and `lean=True`.
- Fitted models expose `fit.options`, an `EstimationOptions` value with the estimation options the fit was built with: the small-sample correction `ssc`, `drop_singletons`, `drop_intercept`, `weights`, `weights_type`, `offset`, `collin_tol`, `solver`, `demeaner` (with its `fixef_tol`), the formulaic `context`, and the storage options `store_data`, `copy_data`, and `lean` (also as `retention`). GLM fits publish a `GlmEstimationOptions` with the IRLS `maxiter`, `tol`, `separation_check`, and `accelerate`; quantile fits a `QuantregEstimationOptions` with `quantile`, `method`, `quantile_tol`, `quantile_maxiter`, and `seed`. It replaces the private attributes `_ssc`, `_drop_singletons`, `_drop_intercept`, `_weights_name`, `_weights_type`, `_has_weights`, `_offset_name`, `_collin_tol`, `_solver`, `_demeaner`, `_fixef_tol`, `_fixef_maxiter`, `_store_data`, `_copy_data`, `_lean`, `_context`, `_quantile`, `_quantile_tol`, `_quantile_maxiter`, `_seed`, and `_accelerate`, and the public `maxiter`, `tol`, and `separation_check` attributes of GLM fits. It survives `store_data=False` and `lean=True`, so the storage policy stays readable after cleanup.
- The model classes (`Feols`, `Feiv`, `Feglm` and its families, `Quantreg`, `QuantregMulti`) take the estimation options as a single `options=` constructor argument, built once by the estimation planner, instead of fifteen loose keyword arguments.
- Fitted models expose `fit.capabilities`, a `Capabilities` value declaring which inference and post-estimation features the model class supports: `crv3_inference`, `hac_inference`, `multiway_clustering`, `wildboottest`, `cluster_causal_variance`, and `decomposition`. It replaces the private attributes `_support_crv3_inference`, `_support_hac_inference`, `_support_multiway_clustering`, `_supports_wildboottest`, `_supports_cluster_causal_variance`, `_support_decomposition`, and the unread `_support_iid_inference`, and survives `store_data=False` and `lean=True`. IV fits now report `wildboottest=False` and are rejected by `wildboottest()` at the capability guard; previously they claimed support and were rejected later.
- `did2s()` fits now report `wildboottest=False`, `cluster_causal_variance=False`, and `decomposition=False`: the two-step GMM covariance does not resample from an estimated model the way `wildboottest()`, `ccv()`, and `decompose()` require. Previously these fits inherited the underlying second-stage OLS fit's default capabilities, so all three methods ran to completion on it as though it were an ordinary single-stage fit, silently ignoring the first-stage residualization.
- `did2s()` fits now reject `predict()` (`prediction=False`). Previously it returned the second stage's fitted values of the first-stage-residualized outcome, not predictions of the outcome.
- `Capabilities` gains `prediction`, `fixed_effect_recovery`, `randomization_inference`, and `sherman_morrison_update`, which `predict()`, `fixef()`, `ritest()`, and `update()` check instead of the estimator name. Apart from `did2s()` prediction, supported combinations are unchanged: IV fits reject all four methods, `ritest()` runs on `feols()` fits without instruments and on `fepois()` fits, and `update()` on `feols()` fits without instruments. `quantreg()` and `did2s()` fits, which cannot have fixed effects, report `fixed_effect_recovery=False`. The rejection messages of these four methods and of `decompose()` now name the estimator and the capability, e.g. "predict() is not supported for 'feols' fits with instruments: fit.capabilities.prediction is False."
- `did2s()` fits now report `crv3_inference=False`, so `vcov({"CRV3": ...})` raises `VcovTypeNotSupportedError`. Previously it refit each leave-one-cluster-out sample of the second stage with the Poisson estimator, which failed with a misleading error or, for a positive outcome, returned standard errors from Poisson refits that ignored the first-stage estimation.
- `vcov({"CRV3": ...})` on `event_study()` fits with `estimator="twfe"` or `estimator="saturated"` now refits each leave-one-cluster-out sample with `feols`. Previously it refit with the Poisson estimator, which failed for outcomes with negative values and otherwise silently returned standard errors from Poisson jackknife refits.
- Fitted models expose `fit.model`, a `ModelDescription` value describing what the model estimates: the `formula` and the parsed `fixest_formula`, the estimator `method` and the `inference_dist` of its coefficient tests, `is_iv`, the `model_name` a multiple estimation stores the fit under, the `sample_split_var` and `sample_split_value` (`None` for an unsplit fit), the dependent variable `depvar`, the absorbed `fixed_effects` (with `has_fixef` and the `+`-joined `fixef`), the `i()` `interacted_covariates`, and the formulaic `model_spec`. It replaces the private attributes `_fml`, `_method`, `_is_iv`, `_model_name`, `_sample_split_var`, `_sample_split_value`, `_inference_dist`, `_depvar`, `_has_fixef`, `_fixef`, `_icovars`, and `_model_spec`, and the public `FixestFormula` attribute of fitted models, and survives `store_data=False` and `lean=True`. `event_study()` and `did2s()` record their estimator (`"twfe"`, `"saturated"`, `"did2s"`) as `fit.model.method`, where they previously overwrote `_method`.
...
- Fitted models now retain the formula `ModelMatrix` as read-only formula
  ...
  independently.
- The internal demeaning cache now stores named `float64` arrays instead of a
  `DataFrame`, so cached columns are selected by position rather than rebuilt
  as a frame on every fit, and the returned response and regressor columns
  follow the order the model asked for regardless of the order the cache
  filled up in. Cached data is handed out read-only, so one fit cannot corrupt
  the demeaned columns another model reuses.
- Linear models no longer store `sqrt(w)`-scaled `X` and `Y`. The demeaned
  arrays and the weights are kept separately and weighting happens inside the
  solver; residuals are always on the scale of the dependent variable. GLM
  IRLS working state (working response, design, and weights) is kept separate
  from observation weights.
- `lean=True` now discards the retained GLM working state so its design and
  working-response arrays are released with the other estimation matrices.

### Inference Types

- `ccv()` now rejects weighted models instead of estimating from a mix of
  weighting scales.
- `ccv()` now raises `NotImplementedError` instead of `AssertionError` for
  model types it does not support.
- `Feols.update()` now rejects `inplace=True`. It remains a coefficient-only
...
- For weighted models, `_sumFE` was stored as `sqrt(w) * D @ alpha`, with the
  IRLS working weights in place of `w` for `fepois()` and `feglm()`. It is now
  `D @ alpha`, on the scale of the linear predictor, matching fixest's
  `sumFE`. `predict()` was unaffected: it uses the fixed-effect coefficients,
  which were already estimated on the correct scale.
- `get_performance()` now raises `NotImplementedError` for non-Gaussian
  `feglm()` families and for `quantreg()`. For GLMs it previously returned
  R²-type values computed from IRLS working arrays; for `quantreg()` it was a
  silent no-op that left the measures `NaN`. Gaussian `feglm()` results are
  unchanged.
...
- `fixef()` and `predict(newdata=...)` for GLM and Poisson fits with only fixed
  effects (`Y ~ 1 | f1`) now recover the fixed effects from the linear
  predictor net of the offset, matching fixest. Before, they returned fixed
  effects on the response scale.
- `decompose()` bootstrap replications now draw from independent random streams.
  With `nthreads` other than 1 (including the default), parallel workers repeated
  the same resamples, so confidence intervals rested on only a handful of distinct
  draws and were typically too narrow; a `seed` now gives the same results for
  any `nthreads`. `tidy(alpha=...)` now honors `alpha`
  instead of always returning 95% intervals, so `bootstrap()` no longer takes it.
...
model objects are removed; going forward, this information is collected from the
typed `fit.options.demeaner` via `fit.options.demeaner.fixef_tol` and
`fit.options.demeaner.fixef_maxiter`.
