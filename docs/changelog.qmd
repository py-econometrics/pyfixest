...
- OLS, IV, and GLM fits expose `fit.sandwich`, a `SandwichComponents` value with the weighted `scores`, the `hessian`, and its inverse `bread`; IV fits carry an `IvSandwichComponents` whose `hessian` is the 2SLS Hessian and whose `projection` maps instrument-space meats into coefficient space. It replaces the private `_scores`, `_hessian`, `_bread`, `_tZX`, `_tXZ`, `_tZy`, and `_tZZinv` fields, is dropped by `lean=True`, and is not carried by quantile regressions, whose inference follows R `quantreg`.
