...
- Fitted models expose `fit.sample`, a `SampleInfo` component with the number of observations `n_obs` (the frequency-weight sum, as fixest's `nobs`), the physical row count, the positions of the dropped input rows, and `DroppedRowCounts` by filtering stage (missing, nonfinite, singleton, separation). It replaces the private `_N`, `_N_rows`, and `_na_index` fields and the public `n_separation_na` attribute of fitted models, and the count fields of `ObservationWeights`; `ModelMatrix.na_index` is now `dropped_positions`. The sample survives `store_data=False` and `lean=True`.
