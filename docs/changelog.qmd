...
### Caching Preconditioners

The `within` LSMR backend now caches its additive Schwarz preconditioner so the
expensive setup phase can be amortised across regressions based on the same fixed-effect
design.
...
# Every Feols-family fit exposes the preconditioner(s) that were built.
fit = pf.feols("Y ~ X1 | f1 + f2", data=df, demeaner=LsmrDemeaner())
pre = fit.preconditioners
...
`WithinPreconditioner` instances are pickleable, so the setup phase can also
