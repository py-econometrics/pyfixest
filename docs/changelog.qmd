...
- Replaces mypy with ty for static type checking, using a Python
  3.10-compatible configuration and enforcing the check through the local lint
  workflow and CI. Associated compatibility fixes preserve `Formula.parse()`
  subclasses, restore the deprecated model-matrix path, and support
  Narwhals-compatible data in post-estimation clustered variance calculations.
