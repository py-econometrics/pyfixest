...
model matrices for `predict()` and `fixef()`. This makes the supported syntax and
`formulaic`'s own syntax the same thing, and removes a class of bugs where a
model matrix was rebuilt from `newdata` with different encodings than the one it
was fitted on.
...
- **`predict(newdata=...)` works with `i()`, `^` and `poly()`.** These
  previously raised `NotImplementedError`; the stored `ModelSpec` now encodes
  `newdata` against the levels seen at fit time. Rows carrying a categorical
  level that was not seen during fitting return `NaN`, matching how unseen fixed
  effect levels were already handled. IV models still do not support
  `predict()`.
...
- `^` keeps its `formulaic` meaning outside the fixed effects part of a formula.
  `Y ~ (X1 + X2)^2` expands to `Y ~ X1 + X2 + X1:X2` again; inside `|` it still
  interacts fixed effects, as in `fixest`.
- `predict(newdata=...)` no longer returns all-`NaN` for categorical factors
  built from an expression, such as `C(np.floor(X2))`. Unseen-level detection
  ran against the factor's source columns instead of the values it encodes.
- Transformed endogenous variables parse again: `Y ~ X1 + [log(X2) ~ Z1]` no
  longer raises.
- A `+` inside a transformed dependent variable is no longer read as multiple
  outcomes, so `I(Y + Y2) ~ X1` estimates one model on the summed outcome.
