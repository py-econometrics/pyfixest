...
- `Capabilities` gains `prediction`, `fixed_effect_recovery`, `randomization_inference`, and `sherman_morrison_update`, which `predict()`, `fixef()`, `ritest()`, and `update()` check instead of the estimator name. Supported combinations are unchanged: IV fits reject all four methods, `ritest()` runs on `feols()` fits without instruments and on `fepois()` fits, and `update()` on `feols()` fits without instruments. The rejection messages of these four methods and of `decompose()` now name the capability.
