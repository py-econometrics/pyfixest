...
- `did2s()` fits now report `wildboottest=False` and `cluster_causal_variance=False`: the two-step GMM covariance does not resample from an estimated model the way `wildboottest()` and `ccv()` require. Previously these fits inherited the underlying second-stage OLS fit's default capabilities, so both methods ran to completion on it as though it were an ordinary single-stage fit, silently ignoring the first-stage residualization.
- `Capabilities` gains `prediction`, `fixed_effect_recovery`, `randomization_inference`, and `sherman_morrison_update`, which `predict()`, `fixef()`, `ritest()`, and `update()` check instead of the estimator name. Supported combinations are unchanged: IV fits reject all four methods, `ritest()` runs on `feols()` fits without instruments and on `fepois()` fits, and `update()` on `feols()` fits without instruments. The rejection messages now name the capability.
