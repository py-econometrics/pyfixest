...
- Saturated event studies record the cohort-specific event-time curves in `fit.did_design.cohort_event_times`. The design replaces the private `_res_cohort_eventtime_dict`, `_yname`, `_gname`, `_tname`, `_idname`, and `_att` attributes that every fitted model used to carry, so `aggregate()`, `iplot_aggregate()`, and `test_treatment_heterogeneity()` now raise `AttributeError` instead of `NotImplementedError` on models that are not saturated event studies.
