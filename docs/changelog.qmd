...
- `tidy()`, `confint()`, and `summary()` gain an `inference_type` argument. For
  `confint()` it takes `"regular"` (pointwise, the default), `"simult"`, or `"savi"`. For `tidy()` and `summary()`, it takes `"regular"` or `"savi"`. The
  `"simult"` option is for simultaneous confidence intervals
...
We add support for safe any-time valid inference (SAVI) for linear regression based on following [Lindon et al (2026)](https://arxiv.org/abs/2210.08589).

You can compute SAVI inference inference is available via `tidy()`, `pvalue()`,
