...
Unweighted `feols()` models now support sensitivity analysis following
[Cinelli and Hazlett
(2020)](https://doi.org/10.1111/rssb.12348). Configure an analysis for one
treatment coefficient, inspect its partial $R^2$ and robustness values, and
benchmark omitted confounding against observed covariates:
...
sensitivity.ovb_bounds("X2", kd=[0.5, 1], ky=[1, 2])
_ = sensitivity.plot(benchmark_covariates="X2", kd=[0.5, 1], ky=[1, 2])
```

The implementation matches the numerical definitions in R's `sensemakr`
package. It supports absorbed fixed effects and `store_data=False` by working
from the fitted model matrix. Sensitivity statistics always use IID standard
errors and residual degrees of freedom; models fitted with robust, clustered,
or HAC covariance estimators emit a warning before using those IID inputs.
Weighted, IV, GLM, quantile, and `lean=True` fits are not yet supported.
Contour and extreme-scenario plots can display benchmark or manually supplied
partial-$R^2$ values without refitting the model.
