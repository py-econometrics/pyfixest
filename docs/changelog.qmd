...
- A new `test-release-contract` task replays the public estimator matrix
  against a pinned pyfixest release instead of R, giving fast Python-only
  regression feedback. It mirrors the structure of `tests/test_vs_fixest.py`,
  and its baseline is recorded per platform from the locked release workspace
  in `tests/snapshots/release/` rather than committed. The R suites remain the
  authoritative reference coverage.
...
- `feglm().resid()` now returns the response residual `y - mu` by default,
  matching R's `glm()`. It previously returned the IRLS working residual, which
  is still available as `resid(type="working")`.
