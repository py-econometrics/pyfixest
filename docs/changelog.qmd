...
- `vcov()` publishes `fit.variance_covariance`, a `VarianceCovariance` value with the adjusted `vcov`, the `meat` where a sandwich exists (so that `vcov == bread @ meat @ bread`), the small-sample factors `ssc`, `df_k`, `df_t`, the estimator (`vcov_type`, `vcov_type_detail`), `clustervar`, and the cluster counts `G`. It replaces the private attributes `_vcov`, `_vcov_type`, `_vcov_type_detail`, `_is_clustered`, `_clustervar`, `_G`, `_ssc`, `_df_k`, `_df_t`, and `_cluster_df`. `did2s` fits now record their cluster variable, so `wald_test`, `wildboottest`, and `ccv` treat them as clustered.
