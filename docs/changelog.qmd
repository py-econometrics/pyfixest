...
- Legacy estimator attributes such as `_X`, `_Y`, and `_weights` are now
  getter-only views over the typed state objects; `_Z` and `_endogvar` are
  defined only on IV models. The views prevent alias assignment and deletion,
  while preserving the underlying arrays' mutability. Fixed-effect recovery
  reads canonical observation weights, including for GLMs.
