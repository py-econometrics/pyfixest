...
- Linear models no longer store `sqrt(w)`-scaled `X` and `Y`. The demeaned
  ...
  solver; residuals are always on the scale of the dependent variable.
...
- `get_performance()` now raises `NotImplementedError` for non-Gaussian
  ...
  unchanged.
- `feglm()` and `fepois()` reject `weights_type="fweights"` until
  frequency-weighted GLM inference is verified against a reference
  implementation (#367).
