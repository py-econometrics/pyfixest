...
- `quantreg` fits expose `fit.solution`, a `QuantregSolution` value with the interior point solver's coefficients `beta`, its `has_converged` flag, its `iterations`, and its final primal and dual iterates. It replaces the private attributes `_has_converged`, `_it`, `_x_final`, `_s_final`, `_z_final`, `_w_final`, and `_y_final`, is now also published by the quantile process (except for the Newton-updated quantiles of `multi_method="cfm2"`), and is dropped by `lean=True`.
