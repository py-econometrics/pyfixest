...
- Contributor and coding-agent guidance now treats public compatibility and
  ...
  for multi-layer work, requires explicit approval before any history rewrite,
  and reserves merging for human maintainers. Repository-local skills route
  implementation, numerical validation, verification, review, history
  curation, and PR handoff to authoritative local developer documentation. A
  compact rpy2 suite compares representative `feols`, `fepois`, and `feglm`
  specifications directly with R `fixest`, and `quantreg` with R `quantreg`.
