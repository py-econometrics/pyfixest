...
### Regression Compression Removal

The in-memory regression compression implementation has been removed. The
`use_compression` and `reps` arguments to `feols()` are still accepted for this
release, but `use_compression=True` now raises a `NotImplementedError`. These
arguments will be removed from the API in a future release.

For large-dataset use cases, the
...
### Deprecated Formula Helpers Removal

The deprecated `FixestFormulaParser` and `model_matrix_fixest()` helpers have
been removed. Use `Formula.parse()` and `create_model_matrix()` from the
formula module instead.

### Deprecated Utility Arguments Removal

The old `ssc()` keyword aliases `adj`, `fixef_k`, `cluster_adj`, and
`cluster_df` have been removed. Use `k_adj`, `k_fixef`, `G_adj`, and `G_df`.
The deprecated report relabel helpers `rename_categoricals()` and
`rename_event_study_coefs()` have also been removed; use `labels` and
`cat_template` in plotting/table calls instead.
