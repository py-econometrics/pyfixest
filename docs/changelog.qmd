...
an optional dependency installed with `pip install pyfixest[numba]`. If Numba
is not installed, `MapDemeaner(backend="numba")` and the fast randomization
inference path raise a clear `ImportError`; the default Rust MAP backend and the
slow randomization inference path work without it.
