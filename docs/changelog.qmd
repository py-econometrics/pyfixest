...
- The contributor skills are now pure procedures: every policy rule has one
  home in `AGENTS.md` or `docs/developer/`, the skills reference it instead of
  restating it, and `.claude/skills/` symlinks expose the same `.agents/skills/`
  files to Claude Code as well as Codex. The reorganization preserves reasons
  for deferring checks, the distinction between cumulative and independent
  stack acceptance, and the rule against counting repeat runs as independent
  evidence.
