...
- A Python-only snapshot suite compares public estimator results with pyfixest
  0.60.0. It mirrors the formula, fixed-effect dtype, inference, weight, IV, and
  SSC structure of the `fixest` comparison tests while using only the default
  demeaner. Each platform builds and reuses its own untracked baseline cache.
