...
- Supplemental `vcov(data=...)` requires the full unchanged input frame and selects fitted rows positionally after checking its length and index; estimation now preserves the caller's index even with `copy_data=False`.
...
- `store_data=False` and `lean=True` now also apply to each model of a multiple estimation and to IV first stages, and fitted models no longer keep the shared demeaning cache alive. Methods that need data or components dropped by these options raise `MissingModelDataError`, which names the missing state and the storage option to change, instead of `AttributeError`.
