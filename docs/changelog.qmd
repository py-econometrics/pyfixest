...
- Contributor and coding-agent guidance now treats public compatibility and
  numerical correctness as production concerns, prefers reviewable stacked PRs
  for multi-layer work, requires curated commit histories, and reserves merging
  for human maintainers. Repository-local skills route implementation,
  numerical validation, verification, review, history curation, and PR handoff
  to authoritative local developer documentation. A deterministic verifier now
  maps changed paths to quick, domain-specific, and exhaustive checks and
  records exact statuses and elapsed time. A fixest-first developer harness
  compares named `feols` and `fepois` results, records reproducible
  provenance, and fails on structural or numerical differences.
