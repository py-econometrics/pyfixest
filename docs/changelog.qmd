...
- The `cat_template` argument of `iplot()` is deprecated. It had no effect, as
  coefficients created via `i()` are already named `variable::value`; use `labels`.
...
- `coefplot()` and `iplot()` now apply `labels` passed without `cat_template`, including to each
  variable of an interaction term, and no longer emit one `DeprecationWarning` per coefficient
  ([#1123](https://github.com/py-econometrics/pyfixest/issues/1123)).
