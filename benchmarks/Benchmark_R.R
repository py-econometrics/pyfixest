...
load("BENCHMARK/DATA/base_all_simulations.Rdata")
            ...
            for(r in all_rep){
                ...
                if(fam == "negbin"){
                    ...
                    results_all = rbind(results_all,
                                        data.frame(family=fam, method="fenegbin", n_obs=n_obs, G=g, rep=r,
                                                   time=getime(pt)))
                    ...
                    if(i <= 2){
                        ...
                        results_all = rbind(results_all,
                                            data.frame(family=fam, method="glmnb", n_obs=n_obs, G=g, rep=r,
                ...
                if(fam == "gaussian"){
                    ...
                    results_all = rbind(results_all,
                                        data.frame(family=fam, method="feols", n_obs=n_obs, G=g, rep=r,
                                                   time=getime(pt)))
                    ...
                    results_all = rbind(results_all,
                                        data.frame(family=fam, method="lfe", n_obs=n_obs, G=g, rep=r,
                    ...
                    if(FALSE && g == 1){
                        ...
                        results_all = rbind(results_all,
                                            data.frame(family=fam, method="plm", n_obs=n_obs, G=g, rep=r,
                ...
                if(fam == "logit"){
                    ...
                    results_all = rbind(results_all,
                                        data.frame(family=fam, method="feglm (fixest)", n_obs=n_obs, G=g, rep=r,
                                                   time=getime(pt)))
                    ...
                    if(g == 1){
                        ...
                        results_all = rbind(results_all,
                                            data.frame(family=fam, method="glmmboot", n_obs=n_obs, G=g, rep=r,
                    ...
                    results_all = rbind(results_all,
                                        data.frame(family=fam, method="feglm (alpaca)", n_obs=n_obs, G=g, rep=r,
                ...
                if(fam == "poisson") {
                    ...
                    results_all = rbind(results_all,
                                        data.frame(family=fam, method="fepois", n_obs=n_obs, G=g, rep=r,
                                                   time=getime(pt)))
...
library(data.table)
base = fread("BENCHMARK/DATA/base_10M.csv")
...
for(g in 1:3){
    ...
    for(r in 1:10){
        ...
        res_fixest = feols(ln_y ~ X1, base, fixef = all_fixef[1:g])
        results_all = rbind(results_all,
                            data.frame(family="gaussian", method="feols", n_obs=1e7, G=g, rep=r,
                                       time=getime(pt)))
        cat(".")
        # lfe
        pt = proc.time()
        ...
        results_all = rbind(results_all,
                            data.frame(family="gaussian", method="lfe", n_obs=1e7, G=g, rep=r,
                                       time=getime_errcheck(pt, res_lfe)))
    }
    cat("\n")
}
...
data.table::fwrite(results_all, file = "BENCHMARK/DATA/results_bench_R.txt")
...
load("BENCHMARK/DATA/base_all_diff.Rdata")
...
for(i in 1:4){
    ...
    for(g in 1:3){
        ...
        for(r in 1:10){
            ...
            results_all_diff = rbind(results_all_diff,
                                data.frame(family="gaussian", method="feols", n_obs=10**(3+i), G=g, rep=r,
                                           time=getime(pt)))
            cat(".")
            ...
            if(i < 4){
                ...
                results_all_diff = rbind(results_all_diff,
                                         data.frame(family="gaussian", method="lfe", n_obs=10**(3+i), G=g, rep=r,
                                                    time=getime_errcheck(pt, res_lfe)))
            }
...
data.table::fwrite(results_all_diff, file = "BENCHMARK/DATA/results_diff_bench_R.txt")