...
def test_single_fit_feols(
    ...
    py_nobs = mod.sample.n_effective
    py_resid = mod.resid()
    py_df_k = mod._df_k
    py_df_t = mod._df_t
...
def test_single_fit_feglm(data_fepois, inference, fml, weights, family):
    ...
    if family == "gaussian":
        ...
        assert (mod.sample.n_effective, int(mod._df_k), int(mod._df_t)) == (
            ref.sample.n_effective,
            int(ref._df_k),
            int(ref._df_t),
        )
...
def test_singleton_dropping():
    ...
    nobs_py = fit_py.sample.n_effective
...
def test_ssc(ssc_data, fml, dropna, weights, vcov, k_adj, G_adj, k_fixef, model):
    ...
    py_nobs = py_fit.sample.n_effective
...
def test_inf_dropping(fml, weights):
    ...
    assert int(data.shape[0] - n_zeros) == fit_py.sample.n_effective
