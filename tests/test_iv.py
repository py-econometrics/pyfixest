...
# Define the ivDiag function (assuming it's an existing R function in the environment)
...
@pytest.mark.parametrize("has_weight", [0.0, 1.0])
@pytest.mark.parametrize("adj_vcov", [0.0, 1.0, 2.0])
def test_iv_Fstat_ivDiag(has_weight, adj_vcov, r_results):
    ...
    fit_iv = feols(
        "y ~ 1 + c1 + c2 | d ~ z", data=data, vcov=vcov_detail, weights=weight_detail_py
    )
    fit_iv.IV_Diag()
