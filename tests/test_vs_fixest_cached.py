...
from config_loader import TestConfigLoader, CachedResultsLoader, inference_to_string, weights_to_string
...
def get_python_results(formula, data, inference, weights, dropna, test_type, family=None):
    ...
    ssc_ = ssc(
        adj=ssc_settings["adj"],
        cluster_adj=ssc_settings["cluster_adj"]
    )
    ...
    try:
        ...
        resid_first_5 = py_resid[:5] if len(py_resid) >= 5 else np.concatenate([py_resid, [np.nan] * (5 - len(py_resid))])
        predict_first_5 = py_predict[:5] if len(py_predict) >= 5 else np.concatenate([py_predict, [np.nan] * (5 - len(py_predict))])
...
def test_feols_cached(data_feols, dropna, inference, weights, formula="Y~X1"):
    ...
    # Get Python results
    py_results = get_python_results(formula, data_feols, inference, weights, dropna, "feols")
    ...
    try:
        r_results = cache_loader.get_result("feols", formula, inference_str, weights_str, dropna)
    except KeyError:
        pytest.skip(f"No cached result found for: {formula}, {inference_str}, {weights_str}, {dropna}")

    # Compare core results
    core_fields = ["coef", "se", "pvalue", "tstat", "confint_low", "confint_high",
                   "vcov_00", "nobs", "n_coefs", "dof_k", "df_t"]
    ...
    if inference == "iid" and weights is None and not dropna:
        ...
        predict_fields = ["predict_1", "predict_2", "predict_3", "predict_4", "predict_5"]
...
def test_all_iv_formulas(data_feols, formula, inference, weights, dropna):
    ...
    # Get Python results
    py_results = get_python_results(formula, data_feols, inference, weights, dropna, "iv")
    ...
    try:
        r_results = cache_loader.get_result("iv", formula, inference_str, weights_str, dropna)
    except KeyError:
        pytest.skip(f"No cached result found for: {formula}, {inference_str}, {weights_str}, {dropna}")
...
def test_all_glm_formulas(data_glm, formula, family, inference, dropna):
    ...
    # Get Python results
    py_results = get_python_results(formula, data_glm, inference, None, dropna, "glm", family)

    # Get cached R results
    inference_str = inference_to_string(inference)

    try:
        r_results = cache_loader.get_result("glm", formula, inference_str, "none", dropna, family)
    except KeyError:
        pytest.skip(f"No cached result found for: {formula}, {family}, {inference_str}, dropna={dropna}")
...
def test_all_fepois_formulas(data_fepois, formula, inference, weights, dropna):
    ...
    # Get Python results
    py_results = get_python_results(formula, data_fepois, inference, weights, dropna, "fepois")
    ...
    try:
        r_results = cache_loader.get_result("fepois", formula, inference_str, weights_str, dropna)
    except KeyError:
        pytest.skip(f"No cached result found for: {formula}, {inference_str}, {weights_str}, {dropna}")
...
def test_cache_validity():
    ...
    assert cache_loader.is_cache_valid(), "Cached results not found. Run tests/scripts/generate_r_results.R first."
