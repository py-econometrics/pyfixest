...
def test_materializes_fixed_effect_specifications(
    ...
    if expected_slopes:
        assert model_matrix.fixed_effect_slopes is not None
        assert model_matrix.fixed_effect_slopes.columns.tolist() == expected_slopes
    else:
        assert model_matrix.fixed_effect_slopes is None
...
def test_no_fixed_effect_slopes_returns_none(varying_slope_data):
    without_fixed_effects = create_model_matrix(
        Formula.parse("y ~ x")[0], varying_slope_data
    )
    with_constant_fixed_effect = create_model_matrix(
        Formula.parse("y ~ x | f1")[0], varying_slope_data
    )

    assert without_fixed_effects.fixed_effect_slopes is None
    assert with_constant_fixed_effect.fixed_effect_slopes is None
