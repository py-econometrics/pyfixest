...
def test_ssc_backward_compatibility_feols(data, k_adj, k_fixef, G_adj, G_df):
    ...
    assert fit_new.covariance.df_t == fit_old.covariance.df_t
    assert fit_new.covariance.df_k == fit_old.covariance.df_k
    np.testing.assert_allclose(
        fit_new.covariance.vcov,
        fit_old.covariance.vcov,
        rtol=0,
        atol=0,
        err_msg="vcov differ",
    )

    assert fit_new.covariance.ssc == fit_old.covariance.ssc
