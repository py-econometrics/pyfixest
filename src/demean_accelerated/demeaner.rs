...
//!
//! It also provides scatter/gather operations for transforming between
//! observation space and coefficient space.
...
pub fn scatter_to_coefficients(ctx: &DemeanContext, values: &[f64]) -> Vec<f64> {
    ...
    scatter_inner(ctx, values, None, &mut result);
    result
}
...
pub fn scatter_residuals(ctx: &DemeanContext, values: &[f64], baseline: &[f64]) -> Vec<f64> {
    let mut result = vec![0.0; ctx.index.n_coef];
    scatter_inner(ctx, values, Some(baseline), &mut result);
    result
}
...
pub fn gather_and_add(ctx: &DemeanContext, coef: &[f64], output: &mut [f64]) {
...
#[inline(always)]
fn scatter_inner(ctx: &DemeanContext, values: &[f64], baseline: Option<&[f64]>, result: &mut [f64]) {
    for q in 0..ctx.index.n_fe {
        let offset = ctx.index.coef_start[q];
        let fe_ids = ctx.index.group_ids_for_fe(q);
...
impl Demeaner for SingleFEDemeaner {
    fn solve(
        ...
        let in_out = scatter_residuals(ctx, input, &output);
...
impl Demeaner for TwoFEDemeaner {
    fn solve(
        ...
        let in_out = scatter_to_coefficients(ctx, input);
...
impl Demeaner for MultiFEDemeaner {
    fn solve(
        ...
        let in_out_phase1 = scatter_to_coefficients(ctx, input);
        ...
        total_iter += iter1;
        gather_and_add(ctx, &coef, &mut mu);

        if !converged1 {
            // Phase 2: 2-FE sub-convergence
            let in_out_phase2 = scatter_residuals(ctx, input, &mu);
            ...
            if remaining > 0 {
                let in_out_phase3 = scatter_residuals(ctx, input, &mu);
                coef.fill(0.0);
