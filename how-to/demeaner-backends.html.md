...
| `LsmrDemeaner(backend="torch", device="cpu")` | LSMR solver on CPU via PyTorch. Good middle ground performance between the MAP and Within demeaners. Requires `pytorch`.|
...
The torch-based LSMR backends requires `pytorch`, which you can install manually or via `pip install pyfixest[torch]`. ; for GPU acceleration on CUDA, you additionally need a CUDA-enabled PyTorch wheel from the [PyTorch installation guide](https://pytorch.org/get-started/locally/).
