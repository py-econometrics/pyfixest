...
It is no longer required to initiate an object of type `Fixest` prior to running [Feols(/docs/reference/Feols.qmd) or `fepois`. Instead,
...
Calling `feols()` will return an instance of class [Feols(/docs/reference/Feols.qmd), while calling `fepois()` will return an instance of class `Fepois`.
...
Visualization is possible via custom methods (`.iplot()` & `.coefplot()`), but a new module allows to visualize
  a list of [Feols(/docs/reference/Feols.qmd) and/or `Fepois` instances:
