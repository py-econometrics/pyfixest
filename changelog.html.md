...
- `MapDemeaner()` (the default, numba-based MAP) for dense fixed-effects problems if you have run the demeaning via JAX on the CPU.
- `LsmrDemeaner(backend="torch", device="cuda")` for GPU acceleration if you have run the demeaning on JAX or Cupy on GPU or scipy on CPU.
