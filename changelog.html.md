...
via `preconditioner="diagonal"`, alongside the existing `"additive"` (default) and
`"off"` options.
...
(potentially) expensive setup phase can be amortised when users explicitly reuse it across
regressions based on the same fixed-effect design.
...
working weights. As a  result, the reused preconditioner turns "stale" - it was built under the initial iteration's weights - but LSMR still converges to the correct demeaned
values; it may just take a few extra iterations per IWLS step. The setup phase
is paid once instead of once per IWLS iteration.

`Preconditioner` instances are pickleable, so the setup phase can also
be amortised across Python sessions. See
